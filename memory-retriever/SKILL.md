...
**Funnel behavior:** MOC-first. The retriever starts by identifying and reading Maps of Content relevant to the query topic. MOC contents provide the structural overview. If MOCs exist and cover the topic, return the MOC-derived map without descending into individual notes. If no relevant MOCs exist, fall back to the full four-stage funnel (Pattern 2 behavior).
